...
insite.contentAdmin = function($) {
    ...
    that.setupFrameLoading = function() {
        History.Adapter.bind(window, "statechange", function () {
            ...
            if (state.data.frameUrl != that.getFrameUrl()) {
                that.loadFrameUrl(state.data.frameUrl);
            }
        });
        ...
        $siteFrame.load(function (e) {
            ...
            that.refreshShell();
            var frameUrl = that.getFrameUrl();
            if (frameUrl.toLowerCase().indexOf("/contentadmin") >= 0)
                return;
            if (History.getState().data.frameUrl != frameUrl) {
                History.pushState({ frameUrl: frameUrl }, null, "?frameUrl=" + frameUrl);
            }
            $.loading.hide();
        });

        //for page reloads, we have to pull the frameUrl out of the query string, History.js doesn't have a way to work with a query string
        var reloadFrameUrl = getQueryString("frameUrl");
        if (reloadFrameUrl !== "") {
        ...
        windowProxy.addEventListener(function (messageEvent) {
            switch (messageEvent.data.action) {
                ...
                case internal.signOut:
                    window.location = getQueryString("frameUrl");
                    break;
                default:
    ...
    var setupPanel = function ($htmlResult, close, update) {
        ...
        if ($ckeditorTextAreas.length > 0) {
            $ckeditorTextAreas.ckeditor(function () {
                var editor = this;
                CKFinder.setupCKEditor(editor, "/console/editor/ckfinder/", "UserFiles");
        ...
        $htmlResult.find("form").submit(function (e) {
            ...
            $iscDateWrap.each(function() {
                ...
                if ($this.find(".isctimepicker").length > 0) {
    ...
    var setupShellEvents = function () {
        $("#cms-shell").on("click", ".cms-shellHeader .btn-edit, .btn-review", function(e) {
            ...
            e.preventDefault();
            windowProxy.post({ action: internal.signOut });
        });
